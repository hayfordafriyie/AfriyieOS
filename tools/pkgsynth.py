...
def make_apk() -> bytes:
    ...
    on Alpine. The two formats have nothing in common beyond the extension, and
    conflating them is a mistake that looks harmless in a file-type table and
    produces a package manager that installs Android apps onto Alpine systems.

    THE STRUCTURE IS CONCATENATED GZIP MEMBERS, which is the detail that makes
    this worth a fixture. A .apk is not one gzip stream containing one tar; it is
    the signature tar, the control tar and the data tar, each independently
    gzipped, written one after another into the same file. A reader that inflates
    one member and stops gets the signature and nothing else.
    """
    pkginfo = (
    ...
    control = build_tar([(".PKGINFO", pkginfo.encode(), 0o644)])
    ...
    # Three members, concatenated: this is the whole point of the fixture.
    return gzip_stored(control) + gzip_stored(data)
