...
def make_deb_zstd() -> bytes:
    """A .deb the way CURRENT dpkg-deb writes one: control.tar.zst, data.tar.zst.

    THE FIXTURE THAT SAYS WHETHER THE ZSTANDARD DECODER MATTERS. Every example of
    a .deb written down for years is a gzip one; `dpkg-deb` changed its default
    underneath the documentation, and a reader that only knows .gz reads the
    wrong member or none at all. This is the same payload as `make_deb`, in the
    container a current Debian or Ubuntu package actually uses, so the two
    results can be compared and any difference is the container's fault.

    Generated by the reference encoder (Python's compression.zstd), so nothing
    here depends on this project's understanding of the format.
    """
    ...
    return build_ar([
        ("debian-binary", b"2.0\n"),
        ("control.tar.zst", zstd.compress(control_tar, options=options)),
        ("data.tar.zst", zstd.compress(data_tar, options=options)),
    ])


def make_deb_bzip2_member() -> bytes:
    """A .deb whose control member uses a compression this build does not decode.

    Used to prove the reader REFUSES BY NAME rather than guessing. A reader that
    treats a bzip2-compressed control member as if it were gzip produces garbage
    metadata and installs a package under the wrong name, which is worse than an
    error.

    THIS FIXTURE USED TO BE xz, AND THE CHANGE IS THE POINT. When the reader
    decided compression from the file extension it refused `control.tar.xz` by
    name, and this was how that refusal was tested. It now decides from the MAGIC
    BYTES, so an xz member is attempted — and testing a by-name refusal with an
    xz member would be testing a behaviour that no longer exists. bzip2 is the
    compression that is genuinely absent.
    """
    return build_ar([
        ("debian-binary", b"2.0\n"),
        ("control.tar.bz2", b"BZh9not really bzip2"),
...
def has_lzma_chunk(blob: bytes) -> bool:
    """True if an .xz stream contains an LZMA-compressed LZMA2 chunk.

    Walks the chunk control bytes only — enough to classify a stream without
    decoding it, which is the point: the generator needs to know which fixtures
    this build can decode before the codec it is testing for exists.

    The 0x00 at the block position is the Index Indicator, which means the stream
    has no more blocks. Reading it as a chunk control byte — which the first
    version of this function did — makes an EMPTY stream look like one full of
    LZMA chunks, and the empty stream is exactly the case where there is nothing
    to walk.
    """
    if len(blob) < 24 or blob[:6] != b"\xfd7zXZ\x00":
        return True

    p = 12
    while p < len(blob):
        if blob[p] == 0x00:
            return False                      # the index: no more blocks

        p += (blob[p] + 1) * 4                # Block Header

        while p < len(blob):
            control = blob[p]
            if control == 0x00:
                break                         # end of this block's LZMA2 stream
            if control >= 0x80 or control > 0x02:
                return True
            size = ((blob[p + 1] << 8) | blob[p + 2]) + 1
            p += 3 + size

        p += 1                                # the end marker

        # Block Padding, then the Check, then the next Block Header.
        while (p % 4) != 0:
            p += 1
        p += {0: 0, 1: 4, 4: 8, 10: 32}.get(blob[7] & 0x0F, 0)

    return False


def write_xz_vectors(out_dir: str) -> None:
    """Writes .xz vectors: the payload, and its stream at several presets.

    THE SAME METHODOLOGY AS THE DEFLATE AND ZSTANDARD VECTORS, and for the same
    reason: an xz decoder that produces output has proved nothing, so every
    vector is compared byte for byte against the exact original.

    WHY THESE PAYLOADS AND NOT OTHERS. This build decodes the .xz container and
    the uncompressed-chunk layer of LZMA2; LZMA-compressed chunks are refused,
    correctly and by name. A fixture containing one therefore cannot be a passing
    vector, and the generator keeps only the streams liblzma encodes as
    uncompressed chunks. The refusal cases are counted and reported rather than
    silently dropped, and tests/native asserts the refusal happens.

    Incompressible data is what produces uncompressed chunks — the encoder emits
    them rather than expanding the data — which is why `random` is the payload
    here. That makes this a real reachable encoding and not a theoretical branch.
    """
    import lzma

    rng = random.Random(0x585A)          # fixed seed: the vectors must not move

    payloads = {
        "xz_random_one": bytes(rng.randrange(256) for _ in range(1)),
        "xz_random_small": bytes(rng.randrange(256) for _ in range(2048)),
        "xz_random_large": bytes(rng.randrange(256) for _ in range(200000)),
        "xz_empty": b"",
        # Compressible payloads, which liblzma encodes as LZMA chunks. They exist
        # so the refusal path has fixtures produced by the reference encoder
        # rather than by hand — a hand-made "invalid" stream tests the parser,
        # not the boundary.
        "xz_text": b"The quick brown fox jumps over the lazy dog. " * 200,
        "xz_zeros": b"\x00" * 100000,
    }
    ...
    for name, payload in payloads.items():
        with open(os.path.join(out_dir, f"{name}.raw"), "wb") as handle:
            handle.write(payload)

        for preset in (0, 1, 6):
            blob = lzma.compress(payload, format=lzma.FORMAT_XZ, preset=preset)

            if has_lzma_chunk(blob):
                # Kept as a refusal case: same payload, and this build must
                # reject it with a precise reason rather than decode it wrongly.
                with open(os.path.join(out_dir, f"{name}.{preset}.lzma.xz"),
                          "wb") as handle:
                    handle.write(blob)
                refused += 1
                continue

            with open(os.path.join(out_dir, f"{name}.{preset}.xz"), "wb") as handle:
                handle.write(blob)
            written += 1

    print(f"  xz vectors:     {len(payloads)} payloads, {written} decodable "
          f"streams, {refused} that must be refused")
...
FIXTURES = {
    ...
    "testzstd_1.2.3-1_amd64.deb": make_deb_zstd,
    "notbz2support_1.0_amd64.deb": make_deb_bzip2_member,
